...
def main():
    ...
    if args.mode == "train":
        ...
        torch.save(model.state_dict(), 'asset/bc_policy_pretrain.pt')
        np.savez('asset/bc_norm_pretrain.npz', a=X_mean, b=X_std, c=Y_mean, d=Y_std)
        ...
        try:
            ...
            for k in range(args.dagger_iters):
                ...
                X_new, Y_new = rollout_dagger_collect(arm, model,(X_mean, X_std, Y_mean, Y_std),device,args.obs_horizon,args.episodes,beta)
            ...
            np.savez('asset/dagger_norm.npz', a=X_mean, b=X_std, c=Y_mean, d=Y_std)
            disconnect_arm(arm)
        ...
        #raise NotImplementedError
        checkpoint = torch.load('asset/dagger_policy.pt', weights_only=True)
        ...
        #raise NotImplementedError
        data = np.load('asset/dagger_norm.npz')
        X_mean = data['a']
        X_std = data['b']
        Y_mean = data['c']
        Y_std = data['d']
