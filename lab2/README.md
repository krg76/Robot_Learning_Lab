...
## Part 1: Understanding Demonstratipython scripts/bc.py --mode train --ip <robot_ip> --epochs <epochs> --batch_size <batch_size> --lr <lr>ons
...
* Are the movements smooth? Not really
* Do actions closely follow the demonstration? Yes
* Are there any large deviations, jerks, or unexpected behavior? 
Yes. For high-freq data, the arm would not move in one direction.
* Start the robot from slightly different initial poses. Observe whether the BC model still produces reasonable behavior
Yes
* Training hyperparameters: epochs 5000, batch_size 256, lr 0.005
* Loss over time. Final training and test loss
Epoch 5000 | Train MSE: 0.000051 | Test MSE: 0.000120
* Visualization of visited EEF states
plot function was disabled due to import errors
...
* How closely does the BC model reproduce the original demonstrations? Closely enough. 
* Where does the model fail or deviate most significantly? Why might that happen? 
After the arm drops off the block, it does not know how to proceed.
* What could go wrong if the robot starts from a pose outside the demonstration distribution?
Distribution shift, so the arm does not know what action to perform at the state that was unseen in the previous demos, which can be dangerous.
* Why is normalization of states and actions important for BC performance? Is this the only way to pre-process data?
If data is not normalized, then an outlier state or action will significantly hurt performance. If it is normalized, then mean squared error from each state-action pair is approximately the same.
