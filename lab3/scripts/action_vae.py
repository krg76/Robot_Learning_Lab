...
class ActionVAE(nn.Module):
    ...
    def encode(self, a):
        ...
        """
        B,H,_ = a.shape
        a = a.flatten(end_dim = 1)

        tmp = self.enc(a)
        mu = self.mu(tmp)
        log_var = self.logvar(tmp)
        mu = torch.reshape(mu,(B,H,-1))
        log_var = torch.reshape(log_var,(B,H,-1))
        return mu,log_var
    ...
    def reparam(mu, logvar):
        ...
        """
        sample = torch.normal(torch.zeros_like(mu),torch.ones_like(logvar))
        return sample*torch.exp(logvar* 0.5) + mu
        #raise NotImplementedError
    ...
    def decode(self, z):
        ...
        """
        B,H,_ = z.shape
        #z_ = torch.flatten(z,start_dim=1)
        #print(z.shape,z_.shape)
        action = self.dec(z)
        #print(z.shape,action.shape)
        #action = torch.reshape(z_1,(B,H,-1))
        return action
